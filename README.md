...
The initial Bedrock implementation lives under [`addons/craftein-core`](./addons/craftein-core). It is an isolated behavior/resource pack scaffold with a server-authoritative player blessing and life-state prototype. Run `addons/craftein-core/validate.sh` to syntax-check and package the current `.mcaddon` artifact. The exact target Bedrock version is still required before production deployment.

The supplied Elemental Weapon and Shounen-style Bedrock pack analysis is recorded in [`docs/craftein-bedrock-reference-study.md`](./docs/craftein-bedrock-reference-study.md). It documents the observed function, scoreboard, skill-entity, attachable, animation, render-controller, particle, sound, and performance patterns without copying third-party content.
