...
client.on('guildMemberAdd', async member => { if (!config.enableMemberEvents) return; const s = store.guild(member.guild.id); const now = Date.now(); const welcome = s.welcome ||= { joinLastAt: 0, joinRecentMemberIds: [], verifiedLastAt: 0, verifiedMemberIds: [] }; const recent = new Set((welcome.joinRecentMemberIds || []).filter(id => id !== member.id)); if (welcome.joinLastAt && now - welcome.joinLastAt < 10 * 60_000) { welcome.joinRecentMemberIds = [...recent, member.id].slice(-20); await store.save(); return; } const channel = member.guild.channels.cache.find(c => c.name === 'welcome' && c.isTextBased()); if (!channel) return; await channel.send(`A new traveler has arrived — welcome, ${member}. Start with the rules, then accept them when you are ready. The world is still being written.`); welcome.joinLastAt = now; welcome.joinRecentMemberIds = [...recent, member.id].slice(-20); await store.save(); });
client.on('interactionCreate', async interaction => {
  try {
    if (interaction.isButton() && interaction.customId === 'show-rules') return respond(interaction, { embeds: [embed('Rules and safety', RULES.map((r, i) => `**${i + 1}.** ${r}`).join('\\n\\n'))], ephemeral: true });
    if (interaction.isButton() && interaction.customId === 'accept-rules') { const role = interaction.guild.roles.cache.find(r => r.name === 'Verified Explorer'); if (!role || role.position >= interaction.guild.members.me.roles.highest.position) return respond(interaction, { content: 'The Verified Explorer role is not currently assignable. Please ask a server administrator to move Craftacus above it.', ephemeral: true }); await interaction.member.roles.add(role, 'Accepted Craftein rules'); const s = store.guild(interaction.guildId); s.members[interaction.user.id] = { ...(s.members[interaction.user.id] || {}), acceptedAt: new Date().toISOString() }; await store.save(); if (db) await db.markRulesAccepted(interaction.guildId, interaction.user.id, new Date()); await logAction(interaction.guild, config, 'Rules accepted', [{ name: 'Member', value: `${interaction.user.tag} (${interaction.user.id})` }]); await greetVerifiedMember(interaction.member); return respond(interaction, { content: 'You are in. Welcome to the Founding Season. Text participation is fully welcome; choose any optional interests below.', components: [rolePicker()], ephemeral: true }); }
    if (interaction.isStringSelectMenu() && interaction.customId === 'lore-menu') { const doc = LORE_DOCS[interaction.values[0]]; if (!doc) return respond(interaction, { content: 'That lore entry is unavailable.', ephemeral: true }); return respond(interaction, { embeds: [embed(doc.label, doc.body)] , ephemeral: true }); }
    if (interaction.isStringSelectMenu() && interaction.customId === 'interest-roles') { if (!requireVerified(interaction)) return respond(interaction, { content: 'Accept the rules first.', ephemeral: true }); const selectable = new Set(INTERESTS.map(([name]) => name)); const chosen = new Set(interaction.values); for (const [name] of selectable) { const role = interaction.guild.roles.cache.find(r => r.name === name); if (role && role.position < interaction.guild.members.me.roles.highest.position) { if (chosen.has(name)) await interaction.member.roles.add(role); else await interaction.member.roles.remove(role); } } return respond(interaction, { content: chosen.size ? `Updated optional interests: ${[...chosen].join(', ')}.` : 'Optional interests cleared. Your access and staff/recognition roles were not changed.', ephemeral: true }); }
    if (!interaction.isChatInputCommand()) return;
    const name = interaction.commandName;
    await interaction.deferReply({ ephemeral: true });
    if (name === 'setup') { if (!modOnly(interaction)) return respond(interaction, { content: 'Manage Guild permission is required.', ephemeral: true }); await setupGuild(interaction.guild); return interaction.editReply('Craftein setup checked and repaired missing resources without deleting existing channels or messages.'); }
    if (name === 'accept-rules') { const role = interaction.guild.roles.cache.find(r => r.name === 'Verified Explorer'); if (!role) return respond(interaction, { content: 'Run /setup first.', ephemeral: true }); await interaction.member.roles.add(role, 'Accepted Craftein rules'); store.guild(interaction.guildId).members[interaction.user.id] = { acceptedAt: new Date().toISOString() }; await store.save(); if (db) await db.markRulesAccepted(interaction.guildId, interaction.user.id, new Date()); await greetVerifiedMember(interaction.member); return respond(interaction, { content: 'Rules acknowledged. Welcome to Craftein.', ephemeral: true }); }
    if (['verify', 'profile'].includes(name) && !requireVerified(interaction)) return respond(interaction, { content: 'Please accept the rules first with the button in #rules-and-safety or /accept-rules.', ephemeral: true });
    if (name === 'verify') { const profile = { guildId: interaction.guildId, userId: interaction.user.id, gamertag: cleanText(interaction.options.getString('gamertag'), 64), region: cleanText(interaction.options.getString('region'), 64), voicePreference: cleanText(interaction.options.getString('voice'), 64) }; if (db) await db.upsertProfile(profile); else { const g = store.guild(interaction.guildId); g.members[interaction.user.id] = { ...(g.members[interaction.user.id] || {}), gamertag: profile.gamertag, region: profile.region, voice: profile.voicePreference, updatedAt: new Date().toISOString() }; await store.save(); } return respond(interaction, { content: 'Profile saved. This is a self-reported community profile, not proof of Microsoft/Xbox ownership.', ephemeral: true }); }
    if (name === 'whitelist-apply') {
      if (!db) return respond(interaction, { content: 'Whitelist applications are temporarily unavailable because Neon is not configured. Please use the application form in #apply-to-craftein.', ephemeral: true });
      ...
      if (!ageEligible) return respond(interaction, { content: 'The established Craftein community requirement is 16+. You cannot submit this application without confirming eligibility.', ephemeral: true });
      ...
      return respond(interaction, { content: `Application #${application.id} submitted for staff review. This is not automatic approval and does not verify Microsoft/Xbox ownership.`, ephemeral: true });
    }
    if (name === 'applications') {
      if (!modOnly(interaction)) return respond(interaction, { content: 'Manage Guild permission is required.', ephemeral: true });
      if (!db) return respond(interaction, { content: 'Neon is not configured; application records are unavailable.', ephemeral: true });
      const rows = await db.listApplications(interaction.guildId);
      if (!rows.length) return respond(interaction, { content: 'There are no pending whitelist applications.', ephemeral: true });
      return respond(interaction, { embeds: [embed('Pending whitelist applications', rows.map(a => `**#${a.id}** · ${a.gamertag} · ${a.region || 'Region not provided'} · <@${a.discord_user_id}>\n${a.reason}`).join('\n\n'))], ephemeral: true });
    }
    if (name === 'application-review') {
      ...
      if (!application) return respond(interaction, { content: 'That application was not found in this guild or was already unavailable.', ephemeral: true });
      ...
      return respond(interaction, { content: `Application #${application.id} marked ${decision}. The applicant still must accept Discord rules; approval does not bypass onboarding.`, ephemeral: true });
    }
    if (name === 'profile') { const p = db ? await db.getProfile(interaction.guildId, interaction.user.id) : store.guild(interaction.guildId).members[interaction.user.id] || {}; return respond(interaction, { embeds: [embed('Your Craftein profile', `**Bedrock gamertag:** ${p.gamertag || 'Not provided'}\n**Region/time zone:** ${p.region || 'Not provided'}\n**Voice preference:** ${p.voice_preference || p.voice || 'Not provided'}\n\nYou can rerun /verify at any time. This profile is voluntary and minimal.`)], ephemeral: true }); }
    if (name === 'season') return respond(interaction, { embeds: [embed('The Founding Season', FOUNDING_SEASON)] });
    if (name === 'lore') return respond(interaction, { embeds: [embed('Craftein lore archive', 'Lore is optional. Choose an entry to open it privately.')], components: [loreMenu()], ephemeral: true });
    if (name === 'roles') { if (!requireVerified(interaction)) return respond(interaction, { content: 'Accept the rules first.', ephemeral: true }); return respond(interaction, { embeds: [embed('Choose your optional interests', 'You can change these at any time. They do not affect staff, recognition, or community access.')], components: [rolePicker()], ephemeral: true }); }
    if (name === 'apply') return respond(interaction, { content: LINKS.application });
    if (name === 'doc') { const topic = interaction.options.getString('topic'); const map = { rules: RULES.map((r, i) => `**${i + 1}.** ${r}`).join('\n\n'), lore: LORE, application: `[Open the main application](${LINKS.application})`, links: Object.entries(LINKS).map(([k, v]) => `**${k}:** ${v}`).join('\n') }; return respond(interaction, { embeds: [embed(`Craftein ${topic}`, map[topic])] }); }
    if (name === 'help') return respond(interaction, { embeds: [embed('Craftacus help', '`/accept-rules` enter after reading the rules\n`/verify` save a voluntary Bedrock profile\n`/profile` view your profile\n`/apply` open the application\n`/season` read the Founding Season\n`/doc` open rules, lore, application, or links\n`/lore` open optional lore privately\n`/roles` update optional interests privately\n`/status` check the configured Bedrock server\n\nStaff: `/setup`, `/announce`, `/warn`, `/timeout`, `/purge`.')], ephemeral: true });
    if (name === 'status') { const result = await getBedrockStatus(config, store); const body = result.state === 'online' ? `Online and reachable.\n**Players:** ${result.players}/${result.maxPlayers ?? '?'}\n**Version:** ${result.version || 'Unknown'}\n**Observed uptime:** ${humanDuration(result.uptimeMs)}` : result.state === 'not-configured' ? 'Status checking is not configured.' : 'Offline or not reachable right now. The address is intentionally not shown.'; return respond(interaction, { embeds: [embed('Craftein server status', body)] }); }
    if (name === 'announce') { if (!modOnly(interaction)) return respond(interaction, { content: 'Manage Guild permission is required.', ephemeral: true }); const target = interaction.guild.channels.cache.find(c => c.name === 'announcements' && c.isTextBased()) || interaction.channel; const msg = await target.send({ embeds: [embed('Craftein announcement', cleanText(interaction.options.getString('message'), 4000))] }); await logAction(interaction.guild, config, 'Announcement posted', [{ name: 'Staff', value: interaction.user.tag }, { name: 'Message', value: msg.url }]); return respond(interaction, { content: `Posted in ${target}.`, ephemeral: true }); }
    if (name === 'warn') { if (!modOnly(interaction)) return respond(interaction, { content: 'Moderation permission is required.', ephemeral: true }); const member = interaction.options.getMember('member'); const reason = cleanText(interaction.options.getString('reason')); await logAction(interaction.guild, config, 'Member warning', [{ name: 'Moderator', value: interaction.user.tag }, { name: 'Target', value: `${member.user.tag} (${member.id})` }, { name: 'Reason', value: reason }]); return respond(interaction, { content: `Documented a warning for ${member.user.tag}.`, ephemeral: true }); }
    if (name === 'timeout') { if (!modOnly(interaction)) return respond(interaction, { content: 'Moderation permission is required.', ephemeral: true }); const member = interaction.options.getMember('member'); const minutes = interaction.options.getInteger('minutes'); const reason = cleanText(interaction.options.getString('reason')); await member.timeout(minutes * 60_000, reason); await logAction(interaction.guild, config, 'Member timeout', [{ name: 'Moderator', value: interaction.user.tag }, { name: 'Target', value: `${member.user.tag} (${member.id})` }, { name: 'Duration', value: `${minutes} minutes` }, { name: 'Reason', value: reason }]); return respond(interaction, { content: `Timed out ${member.user.tag} for ${minutes} minutes.`, ephemeral: true }); }
    if (name === 'purge') { if (!interaction.memberPermissions?.has(PermissionFlagsBits.ManageMessages)) return respond(interaction, { content: 'Manage Messages permission is required.', ephemeral: true }); const amount = interaction.options.getInteger('amount'); await interaction.channel.bulkDelete(amount, true); await logAction(interaction.guild, config, 'Messages purged', [{ name: 'Moderator', value: interaction.user.tag }, { name: 'Channel', value: interaction.channel.name }, { name: 'Amount requested', value: String(amount) }]); return respond(interaction, { content: `Removed up to ${amount} recent messages.`, ephemeral: true }); }
