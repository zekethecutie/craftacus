...
async function greetVerifiedMember(member) { const s = store.guild(member.guild.id); const now = Date.now(); const welcome = s.welcome ||= { lastAt: 0, recentMemberIds: [] }; const recent = new Set((welcome.recentMemberIds || []).filter(id => id !== member.id)); if (welcome.lastAt && now - welcome.lastAt < 10 * 60_000) { welcome.recentMemberIds = [...recent, member.id].slice(-20); await store.save(); return false; } const channel = member.guild.channels.cache.find(c => c.name === 'general' && c.isTextBased()); if (!channel) return false; await channel.send(`Welcome to the community, ${member}. Your path into Craftein is open — take your time, meet a few people, and make a place that feels like yours.`); welcome.lastAt = now; welcome.recentMemberIds = [...recent, member.id].slice(-20); await store.save(); return true; }
...
client.on('guildMemberAdd', async member => { if (!config.enableMemberEvents) return; const s = store.guild(member.guild.id); const now = Date.now(); const welcome = s.welcome ||= { lastAt: 0, recentMemberIds: [] }; const recent = new Set((welcome.recentMemberIds || []).filter(id => id !== member.id)); if (welcome.lastAt && now - welcome.lastAt < 10 * 60_000) { welcome.recentMemberIds = [...recent, member.id].slice(-20); await store.save(); return; } const channel = member.guild.channels.cache.find(c => c.name === 'general' && c.isTextBased()) || member.guild.channels.cache.find(c => c.name === 'welcome' && c.isTextBased()); if (!channel) return; await channel.send(`A new traveler has arrived — welcome, ${member}. Start with the rules, then accept them when you are ready. The world is still being written.`); welcome.lastAt = now; welcome.recentMemberIds = [...recent, member.id].slice(-20); await store.save(); });
