...
async function greetVerifiedMember(member) { const s = store.guild(member.guild.id); const now = Date.now(); const welcome = s.welcome ||= { joinLastAt: 0, joinRecentMemberIds: [], verifiedLastAt: 0, verifiedMemberIds: [] }; const verifiedMembers = new Set(welcome.verifiedMemberIds || []); if (verifiedMembers.has(member.id)) return false; if (welcome.verifiedLastAt && now - welcome.verifiedLastAt < 10 * 60_000) { verifiedMembers.add(member.id); welcome.verifiedMemberIds = [...verifiedMembers].slice(-100); await store.save(); return false; } const channel = member.guild.channels.cache.find(c => c.name === 'general' && c.isTextBased()); if (!channel) return false; await channel.send(`Welcome to the community, ${member}. Your path into Craftein is open — take your time, meet a few people, and make a place that feels like yours.`); verifiedMembers.add(member.id); welcome.verifiedLastAt = now; welcome.verifiedMemberIds = [...verifiedMembers].slice(-100); await store.save(); return true; }
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton() && interaction.customId === 'accept-rules') { const role = interaction.guild.roles.cache.find(r => r.name === 'Verified Explorer'); if (!role || role.position >= interaction.guild.members.me.roles.highest.position) return interaction.reply({ content: 'The Verified Explorer role is not currently assignable. Please ask a server administrator to move Craftacus above it.', ephemeral: true }); await interaction.member.roles.add(role, 'Accepted Craftein rules'); const s = store.guild(interaction.guildId); s.members[interaction.user.id] = { ...(s.members[interaction.user.id] || {}), acceptedAt: new Date().toISOString() }; await store.save(); if (db) await db.markRulesAccepted(interaction.guildId, interaction.user.id, new Date()); await logAction(interaction.guild, config, 'Rules accepted', [{ name: 'Member', value: `${interaction.user.tag} (${interaction.user.id})` }]); await greetVerifiedMember(interaction.member); return interaction.reply({ content: 'You are in. Welcome to the Founding Season. Text participation is fully welcome; choose any optional interests below.', components: [rolePicker()], ephemeral: true }); }
    ...
    if (name === 'accept-rules') { const role = interaction.guild.roles.cache.find(r => r.name === 'Verified Explorer'); if (!role) return interaction.reply({ content: 'Run /setup first.', ephemeral: true }); await interaction.member.roles.add(role, 'Accepted Craftein rules'); store.guild(interaction.guildId).members[interaction.user.id] = { acceptedAt: new Date().toISOString() }; await store.save(); if (db) await db.markRulesAccepted(interaction.guildId, interaction.user.id, new Date()); await greetVerifiedMember(interaction.member); return interaction.reply({ content: 'Rules acknowledged. Welcome to Craftein.', ephemeral: true }); }
