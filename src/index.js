...
import { BRAND, CHANNELS, DESCRIPTION, FOUNDING_SEASON, INTERESTS, LINKS, LORE, RULES } from './content.js';
...
async function setupGuild(guild) {
  ...
  const panels = [['start:welcome', 'welcome'], ['start:rules-and-safety', 'rules'], ['start:how-craftein-works', 'how'], ['start:apply-to-craftein', 'apply'], ['creative:creator-collab', 'creator'], ['partnerships:partnership-info', 'partnership']];
...
client.on('interactionCreate', async interaction => {
  try {
    ...
    if (interaction.isButton() && interaction.customId === 'accept-rules') { const role = interaction.guild.roles.cache.find(r => r.name === 'Verified Explorer'); if (!role || role.position >= interaction.guild.members.me.roles.highest.position) return interaction.reply({ content: 'The Verified Explorer role is not currently assignable. Please ask a server administrator to move Craftacus above it.', ephemeral: true }); await interaction.member.roles.add(role, 'Accepted Craftein rules'); const s = store.guild(interaction.guildId); s.members[interaction.user.id] = { ...(s.members[interaction.user.id] || {}), acceptedAt: new Date().toISOString() }; await store.save(); await logAction(interaction.guild, config, 'Rules accepted', [{ name: 'Member', value: `${interaction.user.tag} (${interaction.user.id})` }]); return interaction.reply({ content: 'You are in. Welcome to the Founding Season. Text participation is fully welcome; choose any optional interests below.', components: [rolePicker()], ephemeral: true }); }
    ...
    if (name === 'accept-rules') { const role = interaction.guild.roles.cache.find(r => r.name === 'Verified Explorer'); if (!role) return interaction.reply({ content: 'Run /setup first.', ephemeral: true }); await interaction.member.roles.add(role, 'Accepted Craftein rules'); store.guild(interaction.guildId).members[interaction.user.id] = { acceptedAt: new Date().toISOString() }; await store.save(); return interaction.reply({ content: 'Rules acknowledged. Welcome to Craftein.', ephemeral: true }); }
    ...
    if (name === 'verify') { const g = store.guild(interaction.guildId); g.members[interaction.user.id] = { ...(g.members[interaction.user.id] || {}), gamertag: cleanText(interaction.options.getString('gamertag'), 64), region: cleanText(interaction.options.getString('region'), 64), voice: cleanText(interaction.options.getString('voice'), 64), updatedAt: new Date().toISOString() }; await store.save(); return interaction.reply({ content: 'Profile saved. This is a self-reported community profile, not proof of Microsoft/Xbox ownership.', ephemeral: true }); }
    if (name === 'profile') { const p = store.guild(interaction.guildId).members[interaction.user.id] || {}; return interaction.reply({ embeds: [embed('Your Craftein profile', `**Bedrock gamertag:** ${p.gamertag || 'Not provided'}\n**Region/time zone:** ${p.region || 'Not provided'}\n**Voice preference:** ${p.voice || 'Not provided'}\n\nYou can rerun /verify at any time. This profile is voluntary and minimal.`)], ephemeral: true }); }
    ...
    if (name === 'status') { if (!config.bedrockHost) return interaction.reply({ embeds: [embed('Bedrock status', 'Not configured. A Bedrock Dedicated Server host and port must be added to the host environment before Craftacus can query it.')] }); try { const result = await status(config.bedrockHost, config.bedrockPort, { timeout: 3000 }); return interaction.reply({ embeds: [embed('Bedrock status', `Online and reachable.\n**Players:** ${result.players?.online ?? 0}/${result.players?.max ?? '?'}\n**Version:** ${result.version || 'Unknown'}`)] }); } catch { return interaction.reply({ embeds: [embed('Bedrock status', 'Not reachable right now. This is a network query result, not a ban or outage diagnosis.')] }); } }
